...
int main(void)
{
	//values and matrices for main clock code and fills with random values
	int matrix[N][N] = {}; 
	int result[N][N] = {}; 
	int result2[N][N] = {}; 
	...
	//This code is from page 692-693 of the King textbook 
	clock_t start_clock = clock(); //starts the clock
	
	matrix_pow_rec(N, matrix, 50, result); //raises the matrix to the 50th power
	
	printf("The clock ticks for the recursive matrix power function: %lu.\n", clock() - start_clock);  
	
	clock_t start_clock2 = clock(); //starts the clock
	
	matrix_pow(N, matrix, 50, result2); //raises the same matrix to the 50th power
	
	printf("The clock ticks for the non-recursive matrix power function: %lu.\n", clock() - start_clock2);  
